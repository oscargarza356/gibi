    ...
    <Modal showModal={showDemoModal} setShowModal={setShowDemoModal}>
      <div className="w-full overflow-hidden md:max-w-md md:rounded-2xl md:border md:border-gray-100 md:shadow-xl">
        <div className="flex flex-col items-center justify-center space-y-3 bg-white px-4 py-6 pt-8 text-center md:px-16">
          ...
          <h3 className="font-display font-bold text-lime-500 text-xs">{hashText}</h3>
        </div>
      </div>
    </Modal>
  );
...
export function useCreateGIBIModal() {
  ...
  return useMemo(
    () => ({ setShowDemoModal, DemoModal: DemoModalCallback, setModalText, setHashText, setLoadingBar }),
    [setShowDemoModal, DemoModalCallback, setModalText, setHashText, setLoadingBar]
  );
}
